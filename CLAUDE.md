...
- Unity presentation (ADR-0014): the client sees only `GameView` through `IGameSession`; engine events are played one by one by `EventPlayer`, and what they look like lives in `FeedbackProfile` assets, never in code. One `ScriptableObject`/`MonoBehaviour` class per file, named after it.
