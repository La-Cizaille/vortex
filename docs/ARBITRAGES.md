...
| ARB-85 | Gabarit de carte : quel aspect, que montre chaque zone ? | **Futuriste, en métal brossé qui reflète la lumière, avec une légère épaisseur**, d'après une image de référence du designer (inspiration seulement, non versionnée). **Disque en haut à gauche : l'emplacement** (ATK, DEF ; ÉVT et TECH pour les événements et les technologies), en texte en attendant des icônes. **Rond central : une gemme à la couleur de la technologie**, qui rayonne (éteinte pour une carte neutre). **L'illustration dans une fenêtre sertie entre le nom et le texte**, au format actuel 16:9 : aucune image n'est cachée sous un panneau, et les images restent des fichiers déposés dans `Art/Cards/`. **Panneaux en métal clair, texte foncé.** Le bandeau du haut porte le nom, l'onglet du bas l'usage. Choix de mise en œuvre : des liserés à la couleur de la technologie (tour de la carte, fenêtre, disque), qui prennent aussi la couleur qui marque une carte ; 0,04 m d'épaisseur. | `tools/blender/build_card.py`, `Art/Cards3D/Card.fbx`, `CardPrefabSync`, `CardDisplay`, `CardFace`, ASSETS §2 et §3, ADR-0017 |
| ARB-86 | Le disque des cartes : des icônes plutôt que « ATK » / « DEF » ? | **Oui, des icônes épurées**, que le designer n'a pas encore : un jeu de quatre formes d'un seul trait est dessiné par script (pointe de flèche pour ATK, bouclier pour DEF, éclat pour les événements, hexagone à noyau pour les technologies). Une image du même nom déposée dans `Art/Icons/` les remplace ; sans image, le disque reprend le texte court. Le designer a aussi validé le gabarit (ARB-85), les proportions, l'épaisseur, les métaux et la gemme bleue qui tire sur le cyan. | `tools/blender/build_icons.py`, `Art/Icons/Slot_*.png`, `CardFace.BadgeIcon`, `CardDisplay`, `CardPrefabSync`, ASSETS §3 |

## 2026-09-26 : troisième playtest

| # | Question | Décision | Appliqué dans |
|---|---|---|---|
| ARB-87 | Que montrer autour de mon vaisseau, et quand « Fin de tour » s'allume-t-il ? | **Le combo n'apparaît que lorsqu'il est jouable**, au centre sous le vaisseau. **Une action d'équipage n'apparaît que lorsqu'elle est possible** ; elle garde sa place sur l'arc, pour que chaque action soit toujours au même endroit. **« Fin de tour » s'allume dès qu'aucune action d'équipage ne reste possible**, même s'il reste une carte ou un combo : on peut choisir de les garder. | INTERFACE §3.2, §3.4, §3.7, `PlayerControls`, `ActionButton`, `ActionArc` |
| ARB-88 | Une attaque déviée vers un autre joueur se montre-t-elle comme une esquive ? | **Non, elle a son animation dédiée** : le rayon bifurque vers la nouvelle cible. L'esquive reste pour une attaque dont les dégâts sont annulés par un effet, sans que le bouclier y soit pour quelque chose. | ANIMATIONS §2 et §3 |

## 2026-09-26 : quatrième playtest

| # | Question | Décision | Appliqué dans |
|---|---|---|---|
| ARB-89 | Retours sur les animations et la disposition | **Le combo passe par-dessus le vaisseau**, au premier plan (il le traversait). **Le vaisseau du joueur avance** vers la table, pour laisser la place au futur cockpit, et **les vaisseaux grossissent** (joueur ×1,2, adversaires ×1,3). **Deux jets au combo** : un vaisseau a deux réacteurs à l'arrière. **Dégâts : comme une balle au bout d'un élastique**, frappée plus ou moins fort : le vaisseau est projeté librement dans le sens du tir, en tournant, puis l'élastique le ramène, un peu au-delà, et il se stabilise à sa place (sauf s'il est détruit). **Les cartes voyagent avec leur modèle 3D** vers leur destination ; une carte que le joueur a lui-même posée en la glissant ne refait pas de trajet, elle est seulement remise à sa place. **Un critique fait sauter le dé vers la caméra**, qui retombe avec un éclat d'énergie. | INTERFACE §3.2, ANIMATIONS §5, `GameDirector`, `ShipMotion`, `KnockbackFeedback`, `CardFlightFeedback`, `CardTrip`, `CriticalFeedback`, `DieSpinner` |
| ARB-90 | Un cockpit pour le joueur ? | **Oui**, au premier plan, sous son vaisseau, à la place des chiffres actuels : une **jauge de PV** qui se remplit et se vide, avec le chiffre dessus ; une **jauge de bouclier en manomètre**, avec une aiguille ; **deux emplacements de modificateurs** en creux (*sockets*), avec le symbole de l'emplacement ; un **vrai interrupteur** pour la surcharge ; **trois diodes** pour les technologies obtenues ; une **étiquette** au nom du joueur. Les modèles et les matériaux sont faits dans Blender ; le jeu les anime. | INTERFACE §3.2, ANIMATIONS §6 |
| ARB-91 | La surcharge armée reste-t-elle armée après l'action qui aurait pu la dépenser ? Le levier du cockpit restait levé après une attaque surchargée. | **Non.** Armer la surcharge est toujours un choix du joueur, pendant son tour, pour **une** action : dès qu'une commande part, le jeton est désarmé, qu'elle l'ait dépensé ou non. Il n'existe pas de mode « surcharge dès qu'elle est disponible ». Le levier redescend de lui-même. | INTERFACE §3.2, `PlayerControls.Send`, `CockpitDisplay`, test `An_armed_overcharge_is_disarmed_once_the_action_is_sent` |
| ARB-92 | Retours sur le premier cockpit | **Plus petit et plus bas, vaisseau remonté** : le cockpit mordait sur le vaisseau (780 × 203 au lieu de 920 × 240, vaisseau du joueur à 33,5 % de la hauteur au lieu de 31 %). **Manomètre gradué** de 0 à 8, avec une **zone rouge** au 0, allumée quand le bouclier ne protège plus rien (à 0, ou désactivé : bouclier effectif nul d'après le moteur, quel que soit l'effet). **Une diode sous l'interrupteur** montre que la surcharge est disponible (le bouton allumé était trop discret). **Habillage** : métal brossé comme les cartes, verre sur le manomètre, aiguille en métal à pointe rouge, jauge de PV en **tube de verre rempli d'un liquide** (plasma qui vire au sang quand les PV baissent, qui monte et descend avec une lenteur visqueuse). **Une attache en métal** passe par-dessus le coin bas gauche de la carte d'attaque et le coin bas droit de la carte de défense, comme si la carte était branchée. | INTERFACE §3.2, ANIMATIONS §6, ASSETS §1, `tools/blender/build_cockpit.py`, `CockpitDisplay`, `IGameSession.Protection` |
| ARB-93 | Quelle direction artistique et quel univers pour le jeu ? | **Un jeu fun, à l'humour noir, sarcastique et adulte, dans une ambiance de pirates de l'espace.** Pitch : *le dernier grand casse avant la fin des temps*, un vortex qui avale le secteur pendant que les capitaines pillent et font campagne ; l'Élection galactique est une satire. **Registres** : futur usé, rouille et néon pour la 3D ; pulp rétro de propagande pour la 2D. **Limites** : humour noir sur la mort, la cupidité, la bureaucratie, la religion et la politique fictives ; violence cartoon ; aucune personne, aucun parti ni aucune religion réels, aucune blague visant un groupe de personnes, aucun contenu sexuel explicite. **Renommages** du contenu possibles, validés par le designer, dans le cadre de l'équilibrage. **Illustrations des cartes** : générées par le designer avec Stable Diffusion, en dehors du dépôt. | `docs/DIRECTION_ARTISTIQUE.md`, ADR-0020 |
| ARB-94 | Retour sur la première planche de la direction artistique | **Plus sombre, moins cartoon.** Le sarcasme reste, mais l'univers est pris au sérieux : visuels graves, éclairage bas, palette assombrie (suie, os, sang), propagande autoritaire plutôt que pulp coloré, animations lourdes plutôt qu'élastiques, pas de gags visuels. L'humour vient du texte froid posé sur ce monde : *l'image est grave, le texte est cruel*. Précisions de la même revue : la Légion de l'Ordre est la faction sérieuse (space marines) ; la faction verte s'appelle les Adeptes de la Corruption ; emblèmes simples (éclair, pièce, poing, champignon) ; chiffres de bord en terminal vert rétro ; peintures extérieures brillantes, usure aux arêtes. | `docs/DIRECTION_ARTISTIQUE.md` §1, §3, §5, §6, §7 |
| ARB-95 | Réponses sur la planche v2, et retour « pas assez technologique » | **Noirceur de la v2 validée.** **SINISTRA** reste le narrateur, en texte seulement (aucune voix n'existe ; une voix éventuelle viendra avec le son). **Option** pour couper ses commentaires : oui. **Les noms des factions remplacent ceux des technologies** dans le jeu. **Typographies** validées (Anton, Big Shoulders Stencil, IM Fell English, Barlow, Share Tech Mono). **Texte d'ambiance des cartes** : oui, écrit par le designer dans un second temps. **Recul des vaisseaux** plus lourd, amorti plus vite. **Plus technologique** : câbles, valves, tuyaux, électronique et écrans partout. **Les cartes deviennent des modules de vaisseau branchables** : boîtier, connecteur, écran pour l'illustration, plaque ou écran pour le texte ; sockets du cockpit en baies, marché noir en rack. | `docs/DIRECTION_ARTISTIQUE.md` §1, §2.6, §3, §4, §5, §6 |
| ARB-96 | La maquette du module convient-elle ? SINISTRA aura-t-elle une voix ? | **Module validé** tel que sur la planche v3 (boîtier, écran d'illustration, terminal de règle, voyants, fusible, broches). **SINISTRA aura une voix**, avec le lot son : voix de synthèse froide, répliques pré-enregistrées en fichiers (aucune synthèse en ligne pendant la partie), coupée avec ses commentaires, volume à part ; licence de l'outil vérifiée et tracée. | `docs/DIRECTION_ARTISTIQUE.md` §2.6, §5.4, §6.5 bis, §6.8 |
| ARB-97 | Suite de la revue des assets : ordre des lots, module 3D, texte d'ambiance, renommages, textures externes | **Ordre des lots validé** (fondations, tranche verticale, voix en texte, menus, modèles, effets, illustrations, son). **Le module 3D part tout de suite à l'atelier Blender**, en parallèle. **Modèles simples en polygones, mais bien texturés et détaillés.** **Banques de textures externes autorisées** (revient sur ARB-79) : textures CC0 de sources connues (Poly Haven, ambientCG), téléchargées en HTTPS comme simples images, chacune notée avec sa source, sa licence et son empreinte SHA-256 dans `art-src/LICENCES.md` ; les intégrations de téléchargement de Blender MCP restent coupées. **Le champ de texte d'ambiance** entre dans le format du contenu dès les fondations, vide. **Renommages et textes de cartes** : on attend l'équilibrage. | `docs/DIRECTION_ARTISTIQUE.md` §6.3, §8 ; `docs/SECURITY.md` §4 |
| ARB-98 | Validation de la bible ; jusqu'où reprendre les modèles ? | **Bible validée, avec une réserve : l'éclairage ne doit jamais nuire à la lisibilité du jeu.** Vaisseaux, cibles, chiffres, cartes et sélection restent nets quelle que soit l'ambiance. **Tous les modèles peuvent être repris** (vaisseau, carte, cockpit, dé…), aucun n'est intouchable, tant que la lisibilité et les fonctionnalités (pièces et repères nommés que le jeu lit) sont préservées. | `docs/DIRECTION_ARTISTIQUE.md` §1, §6.1, §6.3 |
| ARB-99 | Retour sur le lot 2 : les panneaux de métal brossé | **Trop opaques, trop lourds, le métal brossé fait bon marché.** Les plaques deviennent un cadre fin d'acier avec de petits rivets sur un fond sombre translucide, sans texture : la table se voit à travers. Le cadre du tour en cours devient un simple contour. | `ThemeSprites`, `GameScene` (panneaux, bandeau, boutons, marché) |
| ARB-100 | Retour sur les panneaux allégés | **Pas beau : trop anguleux, encore lourd, certains cadres inutiles.** Panneaux arrondis, fond sombre à peine présent, bord presque invisible autour des joueurs ; **aucun cadre autour du bandeau de manche** (le texte seul sur le ciel) ; plus de rivets. Le tour en cours garde un contour arrondi doré. Principe : un cadre n'existe que s'il sert. | `ThemeSprites`, `GameScene`, bible §6.4 |
| ARB-102 | L'écran des PV et du bouclier d'un adversaire | **Un radar pirate** : ce sont des mesures prises sur l'ennemi, donc un instrument de bord imparfait. Chiffres verts avec une légère lueur, une barre de balayage discrète, et de brèves perturbations (saut et baisse de luminosité, moins de 0,2 s, de loin en loin) qui ne cachent jamais un chiffre. Écran arrondi comme les panneaux. (ARB-101 est pris par la PR de l'atelier Blender.) | `RadarScreen`, `ThemeFonts` (matériau *Terminal Lueur*), `GameScene` |
## 2026-09-26 : atelier Blender, lots de la direction artistique
| # | Question | Décision | Appliqué dans |
|---|---|---|---|
| ARB-101 | Le vaisseau Sillage, repris dans la direction artistique (lot B2), convient-il ? | **Non : remplacé par un vaisseau d'équipage.** Réponse du designer : Sillage est un chasseur individuel, trop petit pour le lore (des équipages de pirates) et trop propre pour la bible ; il manquait deux emplacements de réacteurs et un emplacement d'arme d'où part le tir. Sur deux images d'inspiration (un transport blindé orange marqué, un croiseur industriel rouillé couvert de tuyaux), il choisit : **une silhouette qui mélange les deux** (caissons blindés, et machinerie apparente : tuyaux, antennes, ailettes, bloc moteur à deux grosses tuyères) ; **de grands panneaux peints à la couleur du siège**, écaillés aux arêtes ; **une tourelle dorsale lourde** d'où part le tir ; **un nouveau nom**, proposé par l'atelier : **Carcasse**. Les images de référence servent d'inspiration seulement : ni logo ni marquage repris, non versionnées. | `tools/blender/build_ship_carcasse.py`, `art-src/ships/Ship_Carcasse.blend`, `Art/Ships/Ship_Carcasse.fbx`, `Theme/ShipCatalog`, ASSETS §2, ANIMATIONS §6 |

## 2026-09-26 : pistes après les playtests

Après ses premières parties, le game designer a proposé trois pistes d'équilibrage et trois idées de fonctionnalités, à discuter avant tout développement. Le dossier de conception détaille chaque décision : textes proposés, spécification du moteur, variantes, mesures ([`balance/pistes-apres-playtests.md`](balance/pistes-apres-playtests.md)). Les décisions « à mesurer » ne changent aucune règle tant que le rapport n'est pas lu.

| # | Question | Décision | Appliqué dans |
|---|---|---|---|
| ARB-103 | « Régénération parasitaire (D_018) ne fonctionne pas » : bug, ou mauvaise lecture ? | **Pas de bug** : l'arbitrage actuel ne compense que les pertes de Tourment du porteur. Il perd 1 PV, il en regagne 1, et la carte ne change rien aux chances de victoire (−0,0 pt). **Nouvelle lecture retenue : +1 PV pour chaque PV que les autres joueurs perdent à cause d'un Tourment**, fidèle au texte imprimé (« dégât infligé par des jetons de tourment »). Elle s'applique dès que la brique existe ; la mesure dira s'il faut l'atténuer. | Dossier §1 ; à coder : brique `HealOnTormentLoss` (session principale), puis `ruling` de D_018 |
| ARB-104 | Nouvel arrivage est trop aléatoire et donne un combo gratuit : deux prises dans n'importe quels marchés permettent de former une paire. Que faire ? | **Marchés élargis** : pendant la manche, chaque marché montre 7 cartes au lieu de 5, et chacun ne prend qu'une carte. Autres options écartées : deuxième prise payante, un seul exemplaire de l'événement. À mesurer (combos pendant la manche de l'événement) avant adoption. | Dossier §2 ; à coder : point d'interception « taille des marchés », brique `MarketSizeBonus` |
| ARB-105 | La prime sur le leader (option en réserve, ARB-57) pourrait-elle être une carte ? | **Un événement**, « Avis de recherche » (nom provisoire) : pendant la manche, +2 aux attaques contre le seul joueur qui a le plus de PV, en 2 exemplaires. Formes écartées : carte d'attaque neutre, carte d'une faction. À mesurer avec les indicateurs du lot A. L'option de règle reste en réserve. | Dossier §3 ; à coder : condition d'attaque `TargetIsSoleHpLeader`, nouvelles définitions dans les variantes |
| ARB-106 | Explorer la possibilité de réanimer d'autres joueurs ? | **Explorer une carte « Remorquage »** : usage unique, elle ramène un joueur éliminé à 8 PV, sans modificateur ; pendant la manche suivante, il ne peut pas attaquer son sauveteur. Rien n'est adopté avant la mesure du temps passé hors jeu. En réserve : seconde chance, rançon, fantômes renforcés. | Dossier §4 |
| ARB-107 | Explorer la possibilité de faire des alliances ? | **Explorer une carte « Pacte de non-agression »** : le porteur propose un pacte à un adversaire, qui accepte ou refuse. Si le pacte est conclu, aucun des deux ne peut attaquer ni saboter l'autre jusqu'à la fin du prochain tour du porteur. À juger surtout en partie réelle, puisque les bots ne négocient pas. Écartés pour l'instant : victoire partagée, mode par équipes. | Dossier §5 |
| ARB-108 | Combo : devoir « l'utiliser », c'est-à-dire défausser la paire, n'incite pas à changer de cartes et contredit les bonus des cartes ; le jeu en devient peu fluide et frustrant. Que faire ? | **Point laissé ouvert, rien ne change.** Aucune des options proposées ne convient au designer : pacte automatique, activation sans défausse, synergie continue. | RULES (questions ouvertes), dossier §6 |
