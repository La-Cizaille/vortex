...
| La place des commandes du joueur | Scène `Game`, objets `Joueur` (demi-cercle d'actions, combo, jeton de surcharge), `Marché noir` (boutons du marché) et `Fin de tour` | Déplacer ou redimensionner dans la scène : le code ne fait que les remplir et les allumer. |
