...
| `tools/Capture-Unity.ps1 -Scene Game\|Gallery\|Menu -Out <fichier.png> [-Round N] [-Human] [-Phase …]` | Enregistre une image 1920×1080 de la table (des bots jouent jusqu'à la manche demandée), de la galerie ou du menu. Table : avec `-Human`, le premier siège est une personne, et l'image montre son tour après le marché, pendant (`-Phase Market`), ou pendant la visée d'une attaque avec son aperçu (`-Phase Aim`) ; `-Phase Pause` montre le menu de pause, `-Phase End` la fin de partie, `-Phase Log` le journal ouvert ; `-Human -Phase Decision` la première décision du joueur, prise sur la table ; `-TurnSeconds N` limite les tours, pour voir le temps qui reste. Menu : l'accueil, ou `-Phase Local`, `Dev`, `Options`. Pratique pour vérifier une disposition, une illustration ou un modèle en contexte. |
