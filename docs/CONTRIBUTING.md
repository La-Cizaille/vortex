...
| Le tic des dernières secondes d'un tour limité | `Theme/ThemeSettings`, rubrique *Sons* | Glisser un clip audio dans *Timer Tick*. Sans clip, le jeu génère un bip provisoire. |
...
| `tools/Capture-Unity.ps1 -Scene Game\|Gallery\|Menu -Out <fichier.png> [-Round N] [-Human] [-Phase …]` | Enregistre une image 1920×1080 de la table (des bots jouent jusqu'à la manche demandée), de la galerie ou du menu. Table : avec `-Human`, le premier siège est une personne, et l'image montre son tour après le marché, pendant (`-Phase Market`), ou pendant la visée d'une attaque avec son aperçu (`-Phase Aim`) ; `-Phase Pause` montre le menu de pause, `-Phase End` la fin de partie, `-Phase Log` le journal ouvert ; `-Human -Phase Decision` la première décision du joueur, prise sur la table ; `-Phase Effects [-EffectTime s]` un tir, un combo et une explosion figés à mi-course (ANIMATIONS §5) ; `-TurnSeconds N` limite les tours, pour voir le temps qui reste. Menu : l'accueil, ou `-Phase Local`, `Dev`, `Options`. Pratique pour vérifier une disposition, une illustration ou un modèle en contexte. |
...
**Le vaisseau Carcasse** (ARB-101) se construit aussi par script : forme, chanfreins, dépliage UV, repères, puis les textures calculées par Blender dans `art-src/ships/` (tôles, usure et rouille, crasse, bandes de danger, marquages au pochoir). Changer une proportion, c'est changer un nombre dans `tools/blender/build_ship_carcasse.py` et relancer les deux commandes :
```
