...
| `Rules/` | `GameEngine` (API publique sans état), `Game` (contexte de résolution, découpé en `Game.Actions`, `Game.Flow`, `Game.Commands` et `Game.Attack`), `GameStateValidator` (invariants). **M1.** Aperçus : `GameEngine.Preview` joue une commande sur des parties supposées et en résume l'effet dans un `CommandPreview` (ADR-0018). **M4.5.** |
...
| `Scripts/Session/` | `Vortex.Client` | `IGameSession` : la présentation ne connaît que la vue publique, le joueur qui doit agir, les commandes légales, la soumission d'une commande et l'aperçu d'une commande (ADR-0018). `LocalHotSeatSession` en phase 1 (bots possibles sur tout siège, joués pas à pas), `NetworkSession` en phase 2. |
| `Scripts/Presentation/` | `Vortex.Client` | `EventPlayer` (joue les événements un par un), retours visuels (`FeedbackAsset`, `PauseFeedback`, `PrefabFeedback`), `FeedbackProfile` (quel retour pour quel événement). `CardDisplay` : une carte en **objet 3D** (corps à la couleur de sa technologie, illustration, textes TextMeshPro 3D, jetons de Tourment), remplie depuis un `CardFace` ; `CardAnchor` la garde sur sa place de l'interface, face à la caméra (ADR-0017). `DiceFeedback` et `DiceTray` : les dés d'un lancer, qui roulent puis s'arrêtent sur les valeurs du moteur. Un retour visuel qui s'anime lui-même suit la vitesse de lecture (`IFeedbackStage.PlaybackSpeed`). |
| `Scripts/Presentation/Table/` | `Vortex.Client` | La table de jeu (INTERFACE §3) : `GameDirector` (lance la session, place les sièges, joue les événements, fait jouer les bots), `TableModel` (ce que montre la table, qui suit les événements un par un), `SeatLayout` (ordre des adversaires), `GameLogFormatter` (lignes du journal), et les affichages `SeatDisplay`, `MarketDisplay`, `RoundBanner`, `GameLogDisplay`, `PlaybackControls`, `ScreenAnchor`. Mode test : `CommandPanel` (un bouton par commande légale ou réponse de décision) et `CommandLabels` (leurs libellés, tirés de la vue publique et des textes). Lisibilité : `CardZoom` et `CardHover` (carte agrandie quand on pointe une carte de la table). Gestes (M4.5) : `PlayerControls` traduit chaque geste en l'une des commandes permises par le moteur. Les briques de ces gestes sont `ActionButton` (actions d'équipage autour du vaisseau), `CardDrag` (glisser une carte), `HelpBubble` (aide au survol), et les points d'accroche de `MarketDisplay` (achat) et de `SeatDisplay` (utilisation d'une carte, cibles, survol). `IControlsHost` relie les commandes à la table. Pendant la visée d'une attaque ou d'un sabotage, `PreviewText` met en mots l'aperçu calculé par le moteur, affiché dans la bulle d'aide près de la cible (ADR-0018). |
| `Scripts/Gallery/` | `Vortex.Client` | `GalleryController` : la scène Galerie, pour régler l'apparence sans jouer une partie. |
| `Editor/` | `Vortex.Editor` (éditeur seul) | Création des assets de base (`ProjectAssets`, `ThemeAssets`, `GalleryScene`, `GameScene`, avec `UiBuilder` et `SceneFiles`), règles d'import des illustrations, des icônes et des modèles (`ArtImportRules`), captures de scène en mode batch (`SceneCapture`), configuration de Unity MCP, garde-fou des builds de publication (`ReleaseBuildGuard`). |
...
| `Content/`, `Theme/`, `Prefabs/`, `Scenes/`, `Art/`, `Presentation/Feedback/` | — | Assets : contenu du jeu et textes de l'interface, thème et catalogues, prefabs de carte et de panneau d'adversaire, scènes `Game` (la seule du build) et `Gallery`, illustrations et modèles déposés, profil de retours visuels. |
...
**Ce que montre la table pendant la lecture** : le moteur résout une commande d'un coup. Pendant que ses événements sont joués un par un, `TableModel` les suit : PV, bouclier, surcharge, éliminations, technologies, manche et joueur en cours changent à l'écran quand leur événement passe. Les cartes, les marchés et les effets temporaires se recalent sur la vue publique quand la lecture est finie. Deux exceptions, testées :
...
**Couches d'affichage** (ADR-0017) : l'interface est dessinée par la caméra à 10 unités ; les cartes 3D sont à 6 unités, donc devant ses panneaux ; la carte agrandie est à 3 unités ; le calque de premier plan (*overlay* : dés, futures fenêtres) reste au-dessus de tout.
