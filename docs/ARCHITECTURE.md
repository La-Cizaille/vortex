...
| `Scripts/Presentation/` | `Vortex.Client` | `EventPlayer` (joue les événements un par un), retours visuels (`FeedbackAsset`, `PauseFeedback`, `PrefabFeedback`), `FeedbackProfile` (quel retour pour quel événement). `CardDisplay` : une carte (cadre à la couleur de sa technologie, illustration, textes, jetons de Tourment), remplie depuis un `CardFace`. `DiceFeedback` et `DiceTray` : les dés d'un lancer, qui roulent puis s'arrêtent sur les valeurs du moteur. Un retour visuel qui s'anime lui-même suit la vitesse de lecture (`IFeedbackStage.PlaybackSpeed`). |
