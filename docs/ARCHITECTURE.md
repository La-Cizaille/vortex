...
| `Scripts/Presentation/` | `Vortex.Client` | `EventPlayer` (joue les événements un par un), retours visuels (`FeedbackAsset`, `PauseFeedback`, `PrefabFeedback`), `FeedbackProfile` (quel retour pour quel événement). `CardDisplay` : une carte (cadre à la couleur de sa technologie, illustration, textes, jetons de Tourment), remplie depuis un `CardFace`. |
| `Scripts/Presentation/Table/` | `Vortex.Client` | La table de jeu (INTERFACE §3) : `GameDirector` (lance la session, place les sièges, joue les événements, fait jouer les bots), `TableModel` (ce que montre la table, qui suit les événements un par un), `SeatLayout` (ordre des adversaires), `GameLogFormatter` (lignes du journal), et les affichages `SeatDisplay`, `MarketDisplay`, `RoundBanner`, `GameLogDisplay`, `PlaybackControls`, `ScreenAnchor`. Mode test : `CommandPanel` (un bouton par commande légale ou réponse de décision) et `CommandLabels` (leurs libellés, tirés de la vue publique et des textes). |
