...
| Impact et projection | `Knockback.asset` (`KnockbackFeedback`) | `HpLost` d'une attaque ou d'un renvoi, gradué par la gravité (de 0 à 1 à *Heavy Damage* PV, 10 par défaut) : éclair et étincelles de plus en plus forts, éclats de coque dès un coup moyen, onde de choc pour un coup lourd ; le vaisseau est projeté en arrière, puis secoué et tourné sur ses trois axes de façon désordonnée, et revient toujours à sa place, sauf s'il est détruit | Champ *Sparks* : l'impact, agrandi selon la gravité. Projection, rotation, secousses et durées se règlent |
