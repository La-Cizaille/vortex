...
- **Textures** : PNG, 1024×1024 au plus pour un vaisseau. Elles se nomment d'après le modèle :
  ...
  L'export les copie dans le dossier `<Modèle>.fbm/`, à côté du FBX, y compris celles qui passent par une teinte ou un calcul dans Blender. Unity relie seul la couleur de base et le relief aux matériaux (vérifié le 2026-09-25 ; Carcasse n'utilise pas de texture d'émission, dont le branchement reste à vérifier), puis leur applique des réglages mobiles (mipmaps, ASTC 6×6 sur Android).
...
|---|---|---|---|---|---|
| **Vaisseau** | 1, recoloré par le jeu à la couleur de chaque siège (ARB-76). Des modèles distincts par siège pourront s'ajouter ensuite. **Fait : Carcasse** (ARB-101), un vaisseau d'équipage qui remplace le chasseur Sillage : 4 142 triangles, 2,2 m de long ; caissons blindés à grands panneaux peints, machinerie apparente, tourelle dorsale, deux tuyères ; vaisseau par défaut de `Theme/ShipCatalog` | Un par joueur autour de la table ; celui du joueur est au premier plan | Environ 2 m de long, 2,4 m d'envergure au plus (comme le vaisseau provisoire) ; 5 000 triangles au plus. Une **arme visible** d'où part le tir et **deux réacteurs** visibles (ARB-101), marqués par les repères vides `Canon` et `Reacteur…`, que le jeu lit pour leur position (ANIMATIONS §5) | `art-src/ships/Ship_<Nom>.blend` exporté en `Art/Ships/Ship_<Nom>.fbx`, puis associé à un siège dans `Theme/ShipCatalog` | 1 |
...
L'outil d'assets dessine trois images dans `Theme/Sprites/` (`ThemeSprites`) : `Plaque.png` (panneau arrondi, fond sombre translucide, bord à peine visible, découpé en neuf pour s'étirer ; ARB-100), `Contour.png` (contour arrondi du joueur dont c'est le tour, et tampon de la fin de partie), `Affiche.png` (papier jauni aux bords déchirés, fenêtres des menus), `Ecran.png` (écran de terminal vert), `Danger.png` (bandes jaunes et noires, répétées). Elles habillent les panneaux, le bandeau, les boutons, l'annonce de tour et le marché. Pour les remplacer : déposer une image du même nom, en gardant les bords (26 px pour la plaque, 10 px pour l'écran), ou supprimer l'image et relancer l'outil pour la redessiner.
