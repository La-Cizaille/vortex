...
|---|---|---|---|---|---|
| **Vaisseau** | 1, recoloré par le jeu à la couleur de chaque siège (ARB-76). Des modèles distincts par siège pourront s'ajouter ensuite. **Fait : Carcasse** (ARB-99), un vaisseau d'équipage qui remplace le chasseur Sillage : 4 142 triangles, 2,2 m de long ; caissons blindés à grands panneaux peints, machinerie apparente, tourelle dorsale, deux tuyères ; vaisseau par défaut de `Theme/ShipCatalog` | Un par joueur autour de la table ; celui du joueur est au premier plan | Environ 2 m de long, 2,4 m d'envergure au plus (comme le vaisseau provisoire) ; 5 000 triangles au plus. Une **arme visible** d'où part le tir et **deux réacteurs** visibles (ARB-99), marqués par les repères vides `Canon` et `Reacteur…`, que le jeu lit pour leur position (ANIMATIONS §5) | `art-src/ships/Ship_<Nom>.blend` exporté en `Art/Ships/Ship_<Nom>.fbx`, puis associé à un siège dans `Theme/ShipCatalog` | 1 |
