...
- Blender MCP en **mode sûr par défaut** (`BLENDER_MCP_SAFE_MODE=1`) : chaque script est contrôlé avant de s'exécuter ; accès directs aux fichiers, lancement de programmes et réseau bloqués. **Raccourci d'outil de développement (ARB-83)** : un poste peut lever ce contrôle en définissant la variable d'environnement `BLENDER_MCP_SAFE_MODE=0`, que `.mcp.json` lit (`${BLENDER_MCP_SAFE_MODE:-1}`) ; le dépôt, lui, reste en mode sûr. Sans le mode sûr, un script envoyé à Blender a les droits de l'utilisateur (fichiers, programmes, réseau), comme l'assistant en a déjà par son terminal : le risque ajouté est surtout qu'un contenu lu par l'assistant (page web, fichier, commentaire) le pousse à envoyer un script malveillant. Rien de cet outil n'entre dans un build ; les autres mesures ci-dessous restent ;
...
- textures externes (ARB-97) : seulement des images CC0 de banques connues (Poly Haven, ambientCG), téléchargées en HTTPS à la main ou par script, avec leur empreinte SHA-256 notée ; ce sont des données (PNG, JPG) importées par Unity, jamais du code ni des fichiers `.blend` tiers ;
